...
function main() {
    ...
    if (document.readyState === 'loading') {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (!findAndClickButton()) {
                console.log('Nyaser Maps Downloader: DOMContentLoaded后仍未找到按钮，添加1秒延迟后重试');
                setTimeout(findAndClickButton, 1000);
            }
        });
    } else {
        // 文档已加载完成但按钮仍不存在，可能是动态生成的，添加MutationObserver
        console.log('Nyaser Maps Downloader: 文档已加载但按钮不存在，添加MutationObserver监听DOM变化');
        const observer = new MutationObserver(function (mutations) {
            if (findAndClickButton()) {
                console.log('Nyaser Maps Downloader: 按钮通过MutationObserver找到并点击');
                observer.disconnect();
            }
        });
