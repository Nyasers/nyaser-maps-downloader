!(function () {
  function replace() {
    const qrBtn = document.querySelectorAll(
  ...
  try {
    if (
      ...
      (typeof window.__TAURI__ === "undefined" && window.isTauri !== true)
    )
      throw new Error();
    ...
    waitForTauriApi()
      .then(() => window.__TAURI__.core.convertFileSrc)
      .then((f) => f("plugin/main.js", "asset"))
      .then((s) => decodeURIComponent(s))
      .then((u) => import(u));
  } catch (error) {
    failed = true;
  } finally {
    const setupObserver = setupObserverOrigin.bind(
      null,
